// Fixed effects inconsistency model for normal data with identity link

data {
  int<lower=0> ns_a;
  int<lower=0> nt;
  array[ns_a] int<lower=2> na_a;
  array[ns_a, max(na_a)] real y;
  array[ns_a, max(na_a)] real<lower=0> se;
...
model {
  ...
  for (i in 1:ns_a) {
    for (k in 1:na_a[i]) {
      real theta = mu[i] + d[t_a[i,1], t_a[i,k]];
      if (has_metareg == 1) {
...
generated quantities {
  ...
  for (i in 1:ns_a) {
    for (k in 1:na_a[i]) {
      theta_a[i,k] = mu[i] + d[t_a[i,1], t_a[i,k]];
      if (has_metareg == 1) {
