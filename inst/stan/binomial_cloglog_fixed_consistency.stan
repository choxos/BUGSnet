// Fixed effects consistency model for binomial data with cloglog link
...
data {
  ...
  array[ns_a, max(na_a)] real<lower=0> time;  // follow-up time
...
model {
  ...
  for (i in 1:ns_a) {
    for (k in 1:na_a[i]) {
      real theta = log(time[i,k]) + mu[i] + d[t_a[i,k]] - d[t_a[i,1]];
      if (has_metareg == 1) {
        theta += (beta[t_a[i,k]] - beta[t_a[i,1]]) * x_a[i,k];
      }
      // cloglog: p = 1 - exp(-exp(theta))
