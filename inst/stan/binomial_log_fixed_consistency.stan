// Fixed effects consistency model for binomial data with log link
...
parameters {
  vector<lower=0, upper=1>[ns_a] p_base;  // baseline probabilities
...
model {
  ...
  for (i in 1:ns_a) {
    for (k in 1:na_a[i]) {
      real log_p = mu[i] + d[t_a[i,k]] - d[t_a[i,1]];
      if (has_metareg == 1) {
