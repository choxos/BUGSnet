// Fixed effects inconsistency model for binomial data with logit link
...
parameters {
  ...
  matrix[nt, nt] d_raw;                   // treatment effects matrix
...
transformed parameters {
  ...
  // Treatment effects: d[i,j] is effect of j vs i
  for (i in 1:nt) {
    d[i,i] = 0;  // effect of treatment vs itself is zero
  }
  
  for (c in 1:(nt-1)) {
    for (k in (c+1):nt) {
      d[c,k] = d_raw[c,k];
      d[k,c] = -d[c,k];  // symmetric
    }
  }
...
model {
  ...
  // Prior on treatment effects (only upper triangle)
  for (c in 1:(nt-1)) {
    for (k in (c+1):nt) {
